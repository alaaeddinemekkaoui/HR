...
from ..models import Employee, Direction, Division, Service, Grade, Position
...
class EmployeeForm(forms.ModelForm):
    ...
    def clean_employee_id(self):
        ...
        if self.instance.pk:
            qs = qs.exclude(pk=self.instance.pk)
        if qs.exists():
            raise forms.ValidationError('Un employé avec ce matricule existe déjà.')
        return employee_id
    ...
    def clean_cin(self):
        ...
        if self.instance.pk:
            qs = qs.exclude(pk=self.instance.pk)
        if qs.exists():
            raise forms.ValidationError('Un employé avec ce CIN existe déjà.')
        return cin
    ...
    def clean_hire_date(self):
        ...
        if hire_date and hire_date > timezone.now().date():
            raise forms.ValidationError('La date de recrutement ne peut pas être dans le futur.')
        return hire_date
    ...
    def clean(self):
        ...
        if hors_echelle and echelle:
            raise forms.ValidationError('Un employé ne peut pas avoir à la fois une échelle et être hors échelle.')
        # Organizational consistency
        if division and direction and division.direction_id != direction.id:
            self.add_error('division', 'La division sélectionnée n’appartient pas à la direction choisie.')
        if service:
            if service.division:
                # Service under a division; ensure division/direction match
                if division and service.division_id != division.id:
                    self.add_error('service', 'Le service sélectionné n’appartient pas à la division choisie.')
                if direction and service.division.direction_id != direction.id:
                    self.add_error('service', 'Le service sélectionné n’appartient pas à la direction choisie.')
            elif service.direction:
                # Service attached directly to a direction; division should be empty or consistent
                if division:
                    self.add_error('service', 'Ce service est rattaché directement à une direction; laissez la division vide.')
